(function () {
  ...
  function loadForecast() {
    ...
    request(url, function (response) {
      ...
      for (i = 0; i < daily.time.length; i += 1) {
        ...
        html += '<div class="forecast-day"><span class="weekday">' + forecastWeekday(day) + '</span><span class="mini-icon"><img src="' + forecastImage(daily.weather_code[i]) + '" alt=""></span><span class="high">' + Math.round(daily.temperature_2m_max[i]) + '°</span><span class="low">' + Math.round(daily.temperature_2m_min[i]) + '°</span></div>';
      }
  ...
  function parseNews(xmlText) {
    ...
    try { xml = new DOMParser().parseFromString(xmlText, 'text/xml'); items = xml.getElementsByTagName('item'); } catch (e) { return; }
    newsItems = [];
    ...
    if (newsItems.length) { newsIndex = 0; showNextNews(); }
  }

  function loadNews() {
    var feed = 'https://svenska.yle.fi/rss/senaste-nytt';
    request(feed, parseNews, function () {
      request('https://api.allorigins.win/raw?url=' + encodeURIComponent(feed), parseNews, function () {
